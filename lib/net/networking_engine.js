...
shaka.net.NetworkingEngine.prototype.send_ = function(
  ...
  let sendOperation = backoffOperation.chain(() => {
    ...
    if ((response.uri.indexOf('.mp4') > 0) &&
    (response.uri.indexOf('/ldrm/') > 0) &&
    (response.data.byteLength > 0)) {
      /* Get offset from content range header */
      let offset = 0;
      if (response.headers['content-range'] !== undefined) {
        let rangeHdr = response.headers['content-range'];
        if ((rangeHdr.indexOf('bytes ') === 0) &&
            (rangeHdr.indexOf('-') > 0)) {
          let startOffset = rangeHdr.substring(6, rangeHdr.indexOf('-'));
