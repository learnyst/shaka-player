...
shaka.net.NetworkingEngine = class extends shaka.util.FakeEventTarget {
  ...
  send_(type, request, backoff, index, lastError, numBytesRemainingObj) {
    ...
    const sendOperation = backoffOperation.chain(() => {
      ...
      if ((response.uri.indexOf('.mp4') !== -1) &&
      (response.uri.indexOf('/ndrm/') === -1) &&
      ((response.uri.indexOf('/courses/') !== -1) ||
      (response.uri.indexOf('/mock_tests/') !== -1)) &&
      (response.data.byteLength > 0)) {
        /* Get offset from content range header */
        let offset = 0;
        if (response.headers['content-range'] !== undefined) {
          let rangeHdr = '';
          rangeHdr = response.headers['content-range'].trim();
        ...
        const ecrypt = new window['LstCrypt']();

        response.data = ecrypt['lstcrptDecrypt'](response.data, offset, uri);
      }
