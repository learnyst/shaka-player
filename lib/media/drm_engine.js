...
shaka.media.DrmEngine.prototype.initForRemoval = function(
  ...
  /** @type {!Set} */
  const configsByKeySystem = new Set();
  configsByKeySystem.add({
...
shaka.media.DrmEngine.prototype.createOrLoad = function() {
  let currentKeyId = (this.currentDrmInfo_ && this.currentDrmInfo_.keyIds)
                    ? this.currentDrmInfo_.keyIds[0] : null;

  let offlineSessionId;
  if ((currentKeyId) && (this.currentDrmInfo_.keySystem) &&
       (this.currentDrmInfo_.persistentStateRequired)) {
    offlineSessionId
    = window['_x78x23x90llsg'](this.currentDrmInfo_.keySystem, currentKeyId);
  }

  if ((offlineSessionId) && (this.offlineSessionIds_.length <= 0)) {
    this.offlineSessionIds_ = [offlineSessionId];
    shaka.log.debug('Using offline session ID', offlineSessionId);
  }
  ...
  this.offlineSessionIds_.forEach((sessionId) => {
    return this.loadOfflineSession_(sessionId, initDatas[0].initData);
  });
...
shaka.media.DrmEngine.prototype.newInitData = function(initDataType, initData) {
  ...
  this.createTemporarySession_(initDataType, initData, false);
};
...
shaka.media.DrmEngine.prototype.prepareMediaKeyConfigsForVariants_ = function(
  ...
  for (const variant of variants) {
    for (const info of variant.drmInfos) {
      allDrmInfo.add(info);
      if (info.keySystem.startsWith('com.microsoft.playready')) {
        let newInfo = Object.assign({}, info);
        newInfo.keySystem = 'com.microsoft.playready.recommendation';
        allDrmInfo.add(newInfo);
      }
    }
  }
  ...
  // Order DrmInfos
  const orderedAllDrmInfo = new Set();
  let keySystemOrder = [
  ...
  keySystemOrder.forEach((keySystem) => {
    for (const info of allDrmInfo) {
      if (info.keySystem == keySystem) {
        orderedAllDrmInfo.add(info);
      }
    }
  });

  const persistentConfigs = new Map();
  const nonPersistentConfigs = new Map();

  [true, false].forEach((usePersistentLicenses) => {
    let configs;
    if (usePersistentLicenses) {
      configs = persistentConfigs;
    } else {
      configs = nonPersistentConfigs;
    }
    // Create a config entry for each key system.
    for (const info of orderedAllDrmInfo) {
      /* com.microsoft.playready dont support persistent license.
      So dont add persistent config */
      if (usePersistentLicenses) {
  ...
  persistentConfigs.forEach((value)=>{
    allConfigs.add(value);
  });
  nonPersistentConfigs.forEach((value)=>{
    allConfigs.add(value);
  });
  ...
  for (const variant of variants) {
    ...
    for (const info of variant.drmInfos) {
      allConfigs.forEach((config)=>{
        if (config.label.startsWith(info.keySystem)) {
          goog.asserts.assert(
              config,
...
shaka.media.DrmEngine.prototype.queryMediaKeys_ = function(configsByKeySystem) {
  ...
  configsByKeySystem.forEach((config)=>{
  ...
  [true, false].forEach(function(shouldHaveLicenseServer) {
    configsByKeySystem.forEach((config) => {
      shaka.log.debug('Checking keySystem ', config.label);
      ...
      if (hasLicenseServer != shouldHaveLicenseServer) {
        shaka.log.debug('No license server for keySystem ', config.label);
        return;
      }

      shaka.log.debug('Adding keySystem to chain: ', config.label);
      p = p.catch(function() {
        if (this.isDestroying_) { return; }
        selectedConfig = config;
        shaka.log.debug('Trying to create keySystem ', config.label);
        shaka.log.debug('shouldHaveLicenseServer = ', shouldHaveLicenseServer);
        shaka.log.debug('persistent = ', config.persistentState);
        return navigator.requestMediaKeySystemAccess(config.label, [config]);
      }.bind(this));
    });
  }.bind(this));
  ...
  p = p.then(function(mediaKeySystemAccess) {
    ...
    let sessionTypes = realConfig.sessionTypes;
    let persistentState = sessionTypes ?
        sessionTypes.includes('persistent-license') : false;
    if ((selectedConfig.persistentState == 'required')
      && (persistentState == false)) {
      shaka.log.error('Selected key system has pesistent support, ' +
        'but getConfiguration().sessionTypes doesnot include ' +
         'persistent-license. Disabling persistent-license');
      selectedConfig.persistentState = 'optional';
    }
    ...
    shaka.log.debug('Selected keysystem ', mediaKeySystemAccess.keySystem);
    shaka.log.debug('persist =  ', selectedConfig.persistentState);
 ...
 * @private
 */
shaka.media.DrmEngine.prototype.loadOfflineSession_
                          = function(sessionId, initData) {
  let session;
  try {
...
shaka.media.DrmEngine.prototype.createTemporarySession_ =
    function(initDataType, initData, persistentStateRequired) {
  let session;
  try {
  ...
  this.playerInterface_.netEngine.request(requestType, request).promise
      .then(function(response) {
        ...
        return session.update(response.data).then(function() {
          ...
          let currentKeyId = (this.currentDrmInfo_
            && this.currentDrmInfo_.keyIds)
            ? this.currentDrmInfo_.keyIds[0] : null;

          if ((currentKeyId)
            && (this.currentDrmInfo_.keySystem)
            && (this.currentDrmInfo_.persistentStateRequired)) {
            shaka.log.debug('Storing session ', session.sessionId);
            shaka.log.debug('Session type ', this.currentDrmInfo_.keySystem);
            window['_x78x23x90llsa'](this.currentDrmInfo_.keySystem,
              currentKeyId, session.sessionId, response.licenseExpiryDate);
          }
...
shaka.media.DrmEngine.prototype.supportsVariant = function(variant) {
  ...
  return variant.drmInfos.length == 0 ||
      variant.drmInfos.some((drmInfo) => {
        return keySystem.startsWith(drmInfo.keySystem);
      });
};
...
shaka.media.DrmEngine.fillInDrmInfoDefaults_ = function(
  ...
  if (window.cast && window.cast.__platform__) {
    if (drmInfo.keySystem.startsWith('com.microsoft.playready')) {
