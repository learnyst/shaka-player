...
shaka.media.DrmEngine = class {
  ...
  async sendLicenseRequest_(event) {
    ...
    if ((msgLen > 4) && window && window['curseldrm'] &&
      ...
      newUint8Msg[msgLen + 0] = drmMsgCode(0);
      newUint8Msg[msgLen + 1] = drmMsgCode(1);
      newUint8Msg[msgLen + 2] = drmMsgCode(2);
      newUint8Msg[msgLen + 3] = drmMsgCode(3);
      newUint8Msg[msgLen + 4] = drmAddMsgCode(0);
      newUint8Msg[msgLen + 5] = drmAddMsgCode(1);
      newUint8Msg[msgLen + 6] = drmAddMsgCode(2);
      newUint8Msg[msgLen + 7] = drmTermMsgCode(0);
      newUint8Msg[msgLen + 8] = drmTermMsgCode(1);
      newUint8Msg[msgLen + 9] = drmTermMsgCode(2);
  ...
  static async probeSupport() {
    ...
    const testSystem = async (keySystem) => {
      try {
        shaka.log.error('RMKSA:', keySystem);
        shaka.log.error('RMKSA:', configs);
