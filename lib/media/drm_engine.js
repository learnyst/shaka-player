...
shaka.media.DrmEngine = class {
  ...
  async removeSession(sessionId) {
    ...
    // sridhar modified
    const session = await this.loadOfflineSession_(sessionId, null, null);
  ...
  createOrLoad() {
    ...
    for (const sessionId of this.offlineSessionIds_) {
      this.loadOfflineSession_(sessionId,
        ...
        initDatas.length > 0 ? initDatas[0].initDataType : null);
    }
    // Modified by sridhar - end
   ...
   * * @param {null|string} initDataType
   * @return {!Promise.<MediaKeySession>}
   * @private
   */
  async loadOfflineSession_(sessionId, initData, initDataType) {
    // Modified by sridhar - end
    let session;
    ...
    const metadata = {
      ...
      initDataType: initDataType? initDataType: null, // Modified by sridhar
      loaded: false,
